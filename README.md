...
| **4** | 🗃️ **Import the Database Schema** | Open phpMyAdmin at `http://localhost/phpmyadmin`. Create a new database named `centsible_sample`. Import the provided `.sql` file from the project folder using the **Import** tab. Without this step, the application will crash on login. |
